 ...
 * <ul>
 * <li>marshal = from Message to String (can be used when returning as response
 * using the HL7 MLLP codec)</li>
 * <li>unmarshal = from String to Message (can be used when receiving streamed
 * data from the HL7 MLLP codec). This operation will also enrich the message by
 * adding the MSH fields (MSH-3 to MSH-12) as headers on the message.</li>
 * </ul>
 * <p/>
 * Uses the <a href="http://hl7api.sourceforge.net/index.html">HAPI (HL7
 * API)</a> for HL7 parsing.
 * <p/>
 * Uses the default GenericParser from the HAPI API. This DataFormat <b>only</b>
 * supports both the EDI based HL7 messages and the XML based messages.
 * <p/>
    ...
    implements DataFormat
{
        ...
        throws Exception
    {
        ...
        {
        	if(exchange.getOut().getHeader(entry.getKey()) == null)
            exchange.getOut().setHeader(
                entry.getKey(),
                terser.get(entry.getValue()));
        }
        return message;
    }
